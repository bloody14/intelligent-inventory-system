...
# # @app.get("/")
# # def read_route():
# #     return{"message": "first python backend"}

# # @app.get("/items/{item_id}")
# # def read_item(item_id:int):
# #     return{"item_id":item_id,"status":"fetched successfully"}
...
# # CONNECTINF FASTAPI TO DATABASE
...
#     db_item = models.DBItem(name=item.name, price=item.price, is_offer=item.is_offer, owner=db_user)
#     db.add(db_item)
#     db.commit()
#     db.refresh(db_item)
...
# #route to fetch all items from database
...
# # Read Single Item" Route (With 404 Error Handling)
...
# # A secret key used to sign your JWT tokens. Keep this highly secure in production!
...
#     # 3. If credentials match, generate a JWT token containing their identity
...
def create_item(item: ItemCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        print(f"⚠️ Vector DB Error: Failed to generate embedding. Details: {str(e)}")
